...
//Most of the initializations are in pongboard.hh
...
void ball_thread() {
    ...
    //Keep track of the current position so that you know what to lock
    //before moving the ball
    cur_pos.first=ball->x_;
    cur_pos.second=ball->y_;

    reserve.unlock();

    while (true) {
        my_lock(cur_pos);//Lock all cells around my current position
        int mval = ball->move();

        //Keep track of the old position so that you know what to unlock
        std::pair<int,int> old_pos=cur_pos;
        ...
        if (mval > 0) {
            ...
            // Ball fell: notify all to get a new ball
            balls_fell.notify_all();
            break;
        }
    }
    //The ball fell, put it in the reserve:
    reserve.lock();
...
int main(int argc, char** argv) {
    ...
    //Create the arrays of sync objects
    m_arr=new std::mutex[width*height];
    sticky_blocker=new std::mutex[width*height];
    unsticky=new std::condition_variable[width*height];
        ...
        if (nholes == 0) {
            ...
            while (true) {
                if (nrunning < nthreads) {
                    ...
                    ++nstarted;
                    ++nrunning;
                    usleep(1000);//Not sure if I should remove this or not
                }
                else{
                    //The board has enough balls, block until some ball falls and wakes you up
                    std::unique_lock<std::mutex> guard(fall_blocker);
                    balls_fell.wait(guard);
                }
            }
        }
    } else {
